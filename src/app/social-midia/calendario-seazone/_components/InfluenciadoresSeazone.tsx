...
const NAVY = "#0f1d4e"
const COR  = "#7C3AED" // roxo da planilha (igual vistas-hospedes); dashboard usa NAVY
...
/* Campanhas (tabelas físicas) — somadas no dashboard (Geral) e unificadas numa planilha só */
const DATA_TABS = [
...
const INFLU_TABLES = DATA_TABS.map(t => t.table)
const ADD_TABLE = "influencers_seazone" // "Nova linha" na planilha unificada cai na Seazone
...
const BASE_COLS: ColDef[] = [
  ...
  { key: "valor_trabalho",           label: "Vlr Trabalho",     width: 100, type: "money" },
  { key: "valor_hospedagem",         label: "Vlr Hosp.",        width: 90,  type: "money" },
  ...
  { key: "valor_total_reservas",     label: "Vlr Reservas",     width: 100, type: "money" },
  { key: "conteudo_orcado",          label: "Conteúdo Orçado",  width: 180 },
  { key: "observacoes",              label: "Observações",      width: 180 },
]

/* Colunas de valor (moeda) — normalizadas como R$ ao salvar */
const MONEY_COLS = ["valor_trabalho", "valor_hospedagem", "valor_total_reservas"]

const STATUS_OPTIONS    = ["Contratado", "Não contratado", "Aguardando", "Permuta"]
const CATEGORIA_OPTIONS = ["Influ", "Página"]
...
/* Normaliza um valor de moeda: se tiver dígitos, formata como R$ X.XXX,XX;
   caso contrário (ex: "permuta", "aguardando") ou vazio, mantém o texto original */
function normalizeMoney(val: string): string {
  const s = (val ?? "").trim()
  if (!s || !/\d/.test(s)) return s
  return formatBRL(parseBRL(s))
}
...
function GeralTab() {
  ...
  const allFiltered = Object.values(allRows).flat().filter(rowPassFilter)
  const totalCount = allFiltered.length
  const contratadosCount = allFiltered.filter(r => String(r.status_contrato ?? "").toLowerCase().trim() === "contratado").length
  const naoContratadosCount = allFiltered.filter(r => { const s = String(r.status_contrato ?? "").toLowerCase(); return s.includes("não") || s.includes("nao") }).length

  const cellBorder = "#edf0f3"

  return (
    <div>
      {/* Filtrar por período — compacto (estilo vistas) */}
      <div style={{ background: `${COR}08`, border: `1px solid ${COR}25`, borderRadius: 8, padding: "10px 14px", marginBottom: 12 }}>
        <p style={{ margin: "0 0 6px", fontSize: 10, fontWeight: 700, color: COR, textTransform: "uppercase" as const, letterSpacing: "0.06em" }}>Filtrar por período</p>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" as const }}>
          <div style={{ display: "flex", flexDirection: "column" as const, gap: 2 }}>
            ...
            <select value={filterAno} onChange={e => { setFilterAno(e.target.value); setFilterMes("todos") }}
              style={{ padding: "4px 8px", fontSize: 12, border: `1px solid ${T.border}`, borderRadius: 4, outline: "none", background: "#fff", color: T.cardFg, minWidth: 80 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" as const, gap: 2 }}>
            ...
            <select value={filterMes} onChange={e => setFilterMes(e.target.value)}
              style={{ padding: "4px 8px", fontSize: 12, border: `1px solid ${T.border}`, borderRadius: 4, outline: "none", background: "#fff", color: T.cardFg, minWidth: 130 }}>
              <option value="todos">Todos os meses</option>
              {todosMeses.map(m => <option key={m} value={m}>{m}</option>)}
            </select>
          </div>
          <button onClick={() => { setFilterAno("todos"); setFilterMes("todos") }}
            style={{ marginTop: 14, padding: "4px 10px", background: "transparent", border: `1px solid ${T.border}`, borderRadius: 4, fontSize: 11, cursor: "pointer", color: T.mutedFg }}>
            Todos
          </button>
          <span style={{ marginTop: 14, fontSize: 11, color: T.mutedFg }}>{periodoLabel}</span>
        </div>
      </div>
      ...
      {loading ? <div style={{ padding: 48, textAlign: "center" as const, color: T.mutedFg }}>Carregando...</div> : (
        <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" as const }}>
          {[
            { label: "Total", value: String(totalCount), color: T.cardFg },
            { label: "Contratados", value: String(contratadosCount), color: "#065f46" },
            { label: "Não contratados", value: String(naoContratadosCount), color: "#991b1b" },
          ].map(k => (
            <div key={k.label} style={{ background: "#fff", border: `1px solid ${cellBorder}`, borderRadius: 6, padding: "8px 14px", display: "flex", alignItems: "center", gap: 8, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
              <span style={{ fontSize: 11, color: T.mutedFg }}>{k.label}</span>
              <span style={{ fontSize: 18, fontWeight: 700, color: k.color }}>{k.value}</span>
            </div>
          ))}
          <div style={{ background: "#fff", border: `1px solid ${cellBorder}`, borderRadius: 6, padding: "8px 14px", display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" as const, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
            <div>
              <div style={{ fontSize: 10, color: T.mutedFg, fontWeight: 600, textTransform: "uppercase" as const, marginBottom: 2 }}>Orçamento</div>
              {editOrcamento ? (
                <input autoFocus defaultValue={orcamentoInput}
                  onBlur={e => { setOrcamentoInput(e.target.value); setOrcamento(e.target.value); localStorage.setItem("influencers_orcamento_total", e.target.value); setEditOrcamento(false) }}
                  onKeyDown={e => { if (e.key === "Enter") (e.target as HTMLInputElement).blur() }}
                  style={{ width: 110, padding: "2px 6px", fontSize: 13, border: `2px solid ${COR}`, borderRadius: 3, outline: "none" }} placeholder="ex: 50000" />
              ) : (
                <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                  <span style={{ fontSize: 15, fontWeight: 700, color: COR }}>{totalBudget ? formatBRL(totalBudget) : "—"}</span>
                  <button onClick={() => setEditOrcamento(true)} style={{ background: "none", border: "none", cursor: "pointer", color: T.mutedFg, padding: 0 }}><Pencil size={11} /></button>
                </div>
              )}
            </div>
            <div style={{ borderLeft: `1px solid ${cellBorder}`, paddingLeft: 12 }}>
              <div style={{ fontSize: 10, color: T.mutedFg, fontWeight: 600, textTransform: "uppercase" as const, marginBottom: 2 }}>Utilizado</div>
              <span style={{ fontSize: 15, fontWeight: 700, color: "#991b1b" }}>{formatBRL(totalGasto)}</span>
            </div>
            {totalBudget > 0 && (
              <div style={{ borderLeft: `1px solid ${cellBorder}`, paddingLeft: 12 }}>
                <div style={{ fontSize: 10, color: T.mutedFg, fontWeight: 600, textTransform: "uppercase" as const, marginBottom: 2 }}>Saldo</div>
                <span style={{ fontSize: 15, fontWeight: 700, color: saldoLivre >= 0 ? "#065f46" : "#991b1b" }}>{formatBRL(Math.abs(saldoLivre))}{saldoLivre < 0 ? " ⚠️" : ""}</span>
              </div>
            )}
            {totalBudget > 0 && (
              <div style={{ borderLeft: `1px solid ${cellBorder}`, paddingLeft: 12, minWidth: 120 }}>
                <div style={{ fontSize: 10, color: T.mutedFg, fontWeight: 600, textTransform: "uppercase" as const, marginBottom: 4 }}>
                  Utilização <span style={{ fontWeight: 700, color: barColor }}>{pct}%</span>
                </div>
...
function DataTab({ tables, addTable, cols }: { tables: string[]; addTable: string; cols: ColDef[] }) {
  ...
  useEffect(() => { loadRows() }, [tablesKey])

  const rowByKey = (k: string | null) => rows.find(r => r.__key === k)

  async function loadRows() {
    setLoading(true)
    const all: InfluRow[] = []
    for (const tb of tables) {
      const { data } = await getSupabase().from(tb).select("*").order("ano", { ascending: false }).order("mes")
      ;(data ?? []).forEach(r => {
        const o: InfluRow = {}
    ...
    // ordena o conjunto unificado por ano (desc) e mês
    all.sort((a, b) => {
      const ay = a.ano || "", by = b.ano || ""
      if (ay !== by) return by.localeCompare(ay, "pt-BR", { numeric: true })
      return (a.mes || "").localeCompare(b.mes || "", "pt-BR", { numeric: true })
    })
    setRows(all)
    setLoading(false)
  }
  ...
  async function commitEdit(rowKey: string, key: string, val: string) {
    const row = rowByKey(rowKey); if (!row) return
    const finalVal = MONEY_COLS.includes(key) ? normalizeMoney(val) : val
    setSaving(true)
  ...
  async function addRow() {
    ...
    const { data } = await getSupabase().from(addTable).insert(payload).select().single()
    if (data) {
  ...
  async function duplicateRow(row: InfluRow) {
    ...
    const { data } = await getSupabase().from(row.__table).insert(payload).select().single()
    if (data) {
      const newRow: InfluRow = {}
      Object.entries(data).forEach(([k, v]) => { newRow[k] = v === null ? "" : String(v) })
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 16px", background: "#fff", border: `1px solid ${GS.headerBorder}`, borderBottom: "none", borderRadius: "10px 10px 0 0", flexWrap: "wrap" as const }}>
        ...
        {selectedRow && (
          <>
            <button onClick={() => { const r = filtered.find(r => r.__key === selectedRow); if (r) duplicateRow(r) }}
        ...
        <button onClick={addRow}
          style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6, padding: "8px 18px", background: COR, color: "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
      ...
      >
        <table style={{ borderCollapse: "collapse" as const, fontSize: 13, minWidth: "100%", tableLayout: "fixed" as const }}>
          ...
          <tbody>
            {filtered.map((row, i) => {
              const isSelected = selectedRow === row.__key
              const isHovered  = hoverRow === row.__key
              const isAlt      = i % 2 === 1
              ...
              return (
                <tr
                  key={row.__key}
                  onClick={() => setSelectedRow(isSelected ? null : row.__key)}
                  onMouseEnter={() => setHoverRow(row.__key)}
                ...
                >
                  ...
                  <td style={{ width: 52, padding: 0, borderRight: `1px solid ${GS.cellBorder}`, borderBottom: `1px solid ${GS.cellBorder}`, verticalAlign: "middle" as const, cursor: "default", background: bg, boxShadow: isSelected ? `inset 3px 0 0 ${COR}` : "none" }}>
                    <div style={{ height: 36, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" as const }}>
                      ...
                      <div style={{ display: "flex", gap: 3, transition: "opacity 0.1s", opacity: (isHovered || isSelected) ? 1 : 0, pointerEvents: (isHovered || isSelected) ? "auto" : "none" }}>
                        ...
                        <button onClick={e => { e.stopPropagation(); deleteRow(row.__key) }} title="Excluir"
                  ...
                  {cols.map(col => {
                    ...
                    const isEditing = editCell?.id === row.__key && editCell?.key === col.key
                    ...
                    if (isEditing) return (
                      <td key={col.key} style={{ padding: 0, borderRight: `1px solid ${GS.cellBorder}`, borderBottom: `1px solid ${GS.cellBorder}`, verticalAlign: "top" as const, borderTop: `2px solid ${GS.inputBorder}` }}>
                        <textarea autoFocus value={editVal}
                          ...
                          onBlur={() => setTimeout(() => commitEdit(row.__key, col.key, editVal), 100)}
                          onKeyDown={e => {
                            if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); commitEdit(row.__key, col.key, editVal) }
                            if (e.key === "Escape") setEditCell(null)
                            if (e.key === "Tab") { e.preventDefault(); commitEdit(row.__key, col.key, editVal) }
                          }}
                    ...
                    if (col.type === "status") return (
                      <td key={col.key} style={{ ...cellBase, background: bg, padding: "6px 8px", whiteSpace: "nowrap" as const }}>
                        <select value={val} onChange={e => commitEdit(row.__key, col.key, e.target.value)} onClick={e => e.stopPropagation()}
                    ...
                    if (col.type === "link" && val?.startsWith("http")) return (
                      <td key={col.key} style={{ ...cellBase, background: bg, textAlign: "center" as const, whiteSpace: "nowrap" as const }}
                        onDoubleClick={e => { e.stopPropagation(); setEditCell({ id: row.__key, key: col.key }); setEditVal(val) }}>
                    ...
                    /* Colunas de valor: exibe R$ formatado (dígitos) ou texto (ex: permuta) */
                    if (col.type === "money") {
                      const numeric = /\d/.test(val)
                      return (
                        <td key={col.key} style={{ ...cellBase, background: bg, textAlign: "right" as const, whiteSpace: "nowrap" as const, fontVariantNumeric: "tabular-nums" }} title={val}
                          onDoubleClick={e => { e.stopPropagation(); setEditCell({ id: row.__key, key: col.key }); setEditVal(val) }}>
                          {val
                            ? <span style={{ fontWeight: numeric ? 600 : 400, color: numeric ? GS.text : GS.textMuted }}>{numeric ? formatBRL(parseBRL(val)) : val}</span>
                    ...
                    if (isLongText) return (
                      <td key={col.key}
                        ...
                        onDoubleClick={e => { e.stopPropagation(); setModalEdit({ id: row.__key, key: col.key, label: col.label, val }) }}>
                        {val
...
export function InfluenciadoresSeazone() {
  return (
    <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: "18px 22px", boxShadow: T.elevSm }}>
      ...
      {/* Controle de Influencers (orçamento + planilha) */}
      <div style={{ marginTop: 32, marginBottom: 22 }}>
        <p style={{ fontSize: 15, fontWeight: 700, color: T.cardFg, margin: "0 0 4px" }}>Controle de Influencers</p>
      </div>
