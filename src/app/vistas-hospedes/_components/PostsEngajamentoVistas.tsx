...
export function PostsEngajamentoVistas() {
  ...
  useEffect(() => {
    fetch("/api/posts-vistas", { cache: "no-store" })
      .then(r => r.json())
      .then(d => {
  ...
  return (
    <>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8, marginBottom: 14 }}>
        <label htmlFor="sort-posts" style={{ fontSize: 12, fontWeight: 600, color: T.mutedFg }}>Ordenar por:</label>
        <select
          id="sort-posts"
          value={sortBy}
