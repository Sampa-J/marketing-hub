...
/* Interpreta datas em texto livre. Formatos suportados:
   "04/05 a 06/05/25", "04/08 a 05/08/26", "21/06 a 22/06/2026", "23/10 a 24/10",
...
function parseVisita(s: string, anoPadrao: number): { start: Date; end: Date } | null {
  ...
  // ISO: 2025-09-19 (com ou sem hora)
  ...
  const partes: (Parte | null)[] = s.split(/\s+a\s+/i).map(str => {
    ...
    const soDia = str.trim().match(/^(\d{1,2})$/); // ex.: "10" em "10 a 11/01"
    if (soDia) return { dia: +soDia[1], mes: null, ano: null };
    return null;
  });
  if (partes.some(p => !p)) return null;

  // mês/ano de referência: última parte que tiver explícito (o fim tem prioridade)
...
export function VisitaInfluencersVistas() {
  const [rows, setRows] = useState<InfluRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [currentMonth, setCurrentMonth] = useState(new Date());

  useEffect(() => {
    fetch('/api/vistas-influenciadores', { cache: 'no-store' })
      .then(r => r.json())
  ...
  const visitas = useMemo<Visita[]>(() => {
    const out: Visita[] = [];
    for (const r of rows) {
      if (!r.data_visita || !r.perfil) continue;
      const parsed = parseVisita(r.data_visita, anoPadrao);
      if (!parsed) continue;
      out.push({ perfil: r.perfil, foto: r.foto ?? null, link: r.link_perfil ?? null, start: parsed.start, end: parsed.end });
    }
    return out;
  }, [rows, anoPadrao]);
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: T.cardFg, margin: '0 0 4px' }}>Visita de Influencers</h2>
          <p style={{ fontSize: 13, color: T.mutedFg, margin: 0 }}>Datas de visita dos influencers (da planilha) — {visitas.length} visitas cadastradas</p>
        </div>
                ...
                <div key={i} style={{ height: 170, overflow: 'hidden', borderBottom: `1px solid ${T.cinza50}`, borderRight: `1px solid ${T.cinza50}`, padding: 8, display: 'flex', flexDirection: 'column', background: today ? `${T.pendingBg}80` : !isCurrentMonth ? `${T.cinza50}80` : 'transparent', boxShadow: today ? `inset 0 0 0 2px ${T.primary}30` : 'none' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5, overflow: 'hidden', flex: 1 }}>
                    {dayVisitas.slice(0, 2).map((v, j) => {
                      ...
                      const baseStyle: CSSProperties = { display: 'flex', alignItems: 'center', gap: 7, background: `${COR}14`, border: `1px solid ${COR}33`, borderRadius: 8, padding: '6px 8px', textDecoration: 'none' };
                      return v.link ? (
                    ...
                    {dayVisitas.length > 2 && (
                      <span style={{ fontSize: 11, color: COR, fontWeight: 700, padding: '1px 2px' }}>+{dayVisitas.length - 2} mais</span>
                    )}
                  </div>
                </div>
              );
